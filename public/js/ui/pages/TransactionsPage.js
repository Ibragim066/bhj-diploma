...
class TransactionsPage {
  ...
  constructor(element) {
    if (!element) {
      throw new Error("Недопустимое значение для TransactionsWidget")
    }
   ...
   * */
  registerEvents() {
    const ButtonsDel = document.querySelector('.content-wrapper')
    ButtonsDel.addEventListener('click', (evt) => {
      evt.preventDefault()
      if (evt.target.classList.contains('remove-account')) {
        this.removeAccount()
  ...
  removeAccount() {
    if (this.lastOptions) {
      if (confirm('Вы действительно хотите удалить счет и все транзакции?')) {
